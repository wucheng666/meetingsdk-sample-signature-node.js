...
app.post('/getAccessToken00', (req, res) => {
  var options = {
    ...
    url: `https://zoom.us/oauth/token?grant_type=authorization_code&code=${
      req.body.code
    }&redirect_uri=${encodeURIComponent(process.env.redirectURL)}`,
    headers: {
...
app.post('/getAccessToken0', (req, res) => {
  cors(req, res, () => {
    ...
    let url = `https://zoom.us/oauth/token?grant_type=authorization_code&code=${
      req.body.code
    }&redirect_uri=${encodeURIComponent(process.env.redirectURL)}`
...
app.get('/getAccessToken2', (req, res) => {
  ...
  const url =
    baseUrl +
    `?grant_type=authorization_code&code=${req.query.code}&redirect_uri=${process.env.redirectURL}`
  ...
  const options = {
    ...
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
      Authorization: `Basic Sk9HX3IwOERTdHFxdE5OUWhMcExnOjc3aFpNNGVJUnNodXdrYXhxckZWVWlVaWt1Yk02bUdN`
    },
    data,
    url: baseUrl
  }
